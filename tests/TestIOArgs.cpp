...
TEST_F(TestIOArgs, ParseArgs_EmptyInput_ReturnsDefaultConfig) {
    ...
    EXPECT_TRUE(parsed.is_full_info);
    EXPECT_TRUE(parsed.write_console);
    EXPECT_TRUE(parsed.write_json);
    EXPECT_TRUE(parsed.write_yaml);
    EXPECT_EQ(parsed.json_out_path, "data/out.json");
    EXPECT_EQ(parsed.yaml_out_path, "data/out.yaml");
}
...
TEST_F(TestIOArgs, ParseArgs_CustomOutputs_AppliesFlagsAndPaths) {
    ...
    ASSERT_TRUE(parse_result.has_value());
    const Args& parsed = parse_result.value();
    EXPECT_FALSE(parsed.is_full_info);
    EXPECT_FALSE(parsed.write_console);
    EXPECT_TRUE(parsed.write_json);
    EXPECT_TRUE(parsed.write_yaml);
    EXPECT_EQ(parsed.json_out_path, "tmp/r.json");
    EXPECT_EQ(parsed.yaml_out_path, "tmp/r.yaml");
}
