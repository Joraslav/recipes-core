...
void WriteJsonPayload(const Value& value, const fs::path& out_path) {
    std::string buffer;
...
void WriteProductsJson(std::span<const Product> products,
                       const fs::path& out_path) {
    if (out_path.empty()) {
        throw std::invalid_argument("WriteProductsJson: out_path is empty");
    }
    WriteJsonPayload(BuildProductViews(products), out_path);
}

void WriteRecipesJson(std::span<const Recipe> recipes,
                      const fs::path& out_path) {
    if (out_path.empty()) {
        throw std::invalid_argument("WriteRecipesJson: out_path is empty");
    }
    WriteJsonPayload(BuildRecipeViews(recipes), out_path);
}
